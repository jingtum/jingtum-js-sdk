...
/**
 * Convert an interal epoch to a JavaScript timestamp.
 *
 * JavaScript timestamps are unix epoch in milliseconds.
 */
function toTimestamp(t_epoch) {
  return (t_epoch + 0x386D4380) * 1000;
}

/**
 * Convert a JavaScript timestamp or Date to an interal epoch.
 *
 * JavaScript timestamps are unix epoch in milliseconds.
 */
function fromTimestamp(t_epoch) {
  if (t_epoch instanceof Date) {
    t_epoch = t_epoch.getTime();
  }

  return Math.round(t_epoch / 1000) - 0x386D4380;
}
