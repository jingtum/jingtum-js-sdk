...
FinGate.prototype.setConfig = function(token, sign_key) {
  try{
    if (!token || typeof(token) !== 'string')
      throw new ParamException("Invalid token param!");
    if (!sign_key || typeof(sign_key) !== 'string')
...
FinGate.prototype.issueCustomTum = function(obj,callback) {
  ...
  try{
    if (!obj.custom || typeof(obj.custom) !== 'string')
    ...
    if (!obj.amount || !/^-?\d+\.?\d{2}$/.test(obj.amount)) //控制两位小数以内
    ...
    if (!obj.key || typeof(obj.key) !== 'string')
      throw new ParamException("Invalid key param");
  ...
  request(options, function (err, res, body) {
    if(err) {console.log('err:',err); callback(err);}
      ...
      if(body.code === 0){
        ...
        console.log('err code:',body.code);
...
FinGate.prototype.queryCustomTum = function(obj,callback) {
  ...
  try{
    ...
    if(!obj.date || isNaN(obj.date))
      throw new ParamException("Invalid date param");
    if (!obj.key || typeof(obj.key) !== 'string')
      throw new ParamException("Invalid key param");
  }catch(e){console.log(e);newCallback(e,null);return;}
  obj.url = this._url;
