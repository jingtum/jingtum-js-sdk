 ...
 *
 * The API format:
 * For currency pair /v2/order_book/{:base1}+{:address}/{:base2}+{:address}
 * For SWT and Currency pair
 *  case 1: /v2/order_book/SWT/{:base}+{:address}
 *  case 2: /v2/order_book/{:base}+{:address}/SWT
 * 需要检测输入参数的格式。
...
FinGate.prototype.getOrderBook = function(pair, callback) {
  ...
  var url = this._single.server + this.apiVersion  + ORDERBOOK + '/'+ path;

  request.get(url, function (err,res,body) {
    if(err) {callback(err,null);}
    else if(body) {
      var data = JSON.parse(body);
      if(data.order_book != undefined)
      {
...
function formatOrderBook(bids){
  /*for(var i=0;i<bids.length;i++){
    ...
    //bid.funded = bid.sell ? bid.taker_gets_funded.value : bid.taker_pays_funded.value;
...
FinGate.prototype.setActiveAmount = function (amount) {
  try{
    if(!amount || isNaN(amount))
      ...
      throw new ParamException('The minimum value of amount is 25!');
    }
  }catch(e){
    console.log(e);
    return e;
